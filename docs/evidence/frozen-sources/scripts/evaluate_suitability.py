...
OPTIONS = dict(
    temperature=0, seed=42, num_ctx=8192, num_predict=300,
    presence_penalty=0, repeat_penalty=1, top_k=20, top_p=0.95,
)
...
def prepare():
    ...
    for info in old["images"]:
        c = cases[info["id"]]
        row = dict(
            id=info["id"], path=info["path"], sha256=info["sha256"],
            cohort=cohort(c), identity_group=c["leakage_group"],
            live_dog_present=c["live_dog_present"], expected_label=c["classifier_label"],
            expected_subject_type="real_dog" if c["live_dog_present"] else (
                "dog_depiction" if c["kind"] == "dog_sculpture" else "other_animal"
            ),
            expected_dog_count="multiple" if c["kind"] == "multiple_dogs" else (
                "one" if c["live_dog_present"] else "zero"
            ),
    ...
    for name in NEW_IDS:
        ...
        rows.append(dict(
            id=name, path=str(path.relative_to(ROOT)), sha256=sha(path), cohort="non_dog",
            identity_group="greyfriars-bobby-sculpture" if depiction else (
                "socks" if name.startswith("socks") else "valegro"
            ),
            live_dog_present=False, expected_label=None,
            expected_subject_type="dog_depiction" if depiction else "other_animal",
            expected_dog_count="zero", visual_review=reviews[name],
            source_title=page["title"], source_page=info["descriptionurl"],
    ...
    for row in rows:
        ...
        row.update(
            normalized_metadata=metadata, normalized_path=str(normal_path.relative_to(ROOT)),
            normalized_sha256=sha(normal_path), vision_path=str(vision_path.relative_to(ROOT)),
            vision_sha256=sha(vision_path), inspection=inspect_image(normal_path),
        )
    assert len(rows) == 25 and len({r["normalized_metadata"]["sha256"] for r in rows}) == 25
    assert sum(r["live_dog_present"] for r in rows) == 17
    pairs = [dict(id=c["id"], photo_ids=c["photo_ids"], cohort=cohort(c),
                  expected_label=c["classifier_label"], exposure="Previously exposed pair")
             for c in old["cases"] if len(c["photo_ids"]) == 2]
    pairs.extend([
        dict(id="socks", photo_ids=["cat-01", "socks-02"], cohort="non_dog", expected_label=None,
             exposure="One exposed image; documented second photo of the same cat"),
        dict(id="valegro", photo_ids=["valegro-01", "valegro-02"], cohort="non_dog", expected_label=None,
             exposure="New photos of the same named horse, 2012 and 2014"),
        dict(id="bobby-statue", photo_ids=["bobby-statue-01", "bobby-statue-02"], cohort="non_dog",
             expected_label=None, exposure="New photos of the same sculpture, different authors and exposures"),
    ])
    manifest = dict(
        created_at_utc=now(), images=rows, pairs=pairs,
    ...
    files = [Path(__file__).resolve(), OLD_MANIFEST, VIT_CACHE, RESNET_CACHE, ROOT / "uv.lock",
             ROOT / "docs/paired-suitability-protocol.md", ROOT / "scripts/offline.sb",
    ...
    for model_dir in [ROOT / "models/vit", ROOT / "models/resnet50-candidate"]:
        files.extend(model_dir / name for name in ["config.json", "preprocessor_config.json", "model.safetensors"])
    schedule = sorted(r["id"] for r in rows)
    random.Random(42).shuffle(schedule)
    schedule.append("no-image-control")
    protocol = dict(
        registered_at_utc=now(), manifest_sha256=sha(DIRECTORY / "manifest.json"),
        source_hashes={str(p.relative_to(ROOT)): sha(p) for p in files},
        packages={name: importlib.metadata.version(name) for name in ["torch", "transformers", "pillow", "httpx"]},
        new_classifier_ids=NEW_IDS, vision_schedule=schedule, vision_prompt=PROMPT,
        vision_schema=SCHEMA, vision_options=OPTIONS, vision_timeout_seconds=60,
        ollama_version="0.34.0", model=MODEL, model_digest=CONTROLLER_MODELS[MODEL],
        vit_rule=dict(min_score=0, min_margin=0, minimum_ranked_photos=2,
        ...
        vision_advance=dict(max_non_dog_accepts=0, min_live_dog_accepts=16,
                            min_supported_reports=6, max_error_fraction=0.1,
                            max_multi_dog_individual_scopes=0, require_all_valid=True,
        ...
        app_defaults_changed=False, controller_calls=0,
    )
...
def verify_inputs():
    ...
    for row in manifest["images"]:
        for p, h in [("path", "sha256"), ("normalized_path", "normalized_sha256"),
...
def resnet_worker(conn):
    try:
        ...
        assert torch.backends.mps.is_available(), "The registered comparator uses MPS; no silent device change."
        ...
        assert not loading["missing_keys"] and not loading["unexpected_keys"] and not loading["mismatched_keys"]
        model.eval().to("mps")
        ...
        while True:
            ...
            conn.send(dict(ok=dict(
                native_scores=scores.tolist(), device="mps", inference_seconds=time.monotonic() - started,
                candidates=[dict(class_id=int(i), label=model.config.id2label[int(i)], score=float(s))
                            for s, i in zip(values, indices)],
                top1_score=float(values[0]), margin=float(values[0] - values[1]),
...
def resnet_reports(row):
    return bool("error" not in row and row["native_top1_is_breed"] and row["top1_score"] >= 0.8
                and row["margin"] >= 0)
...
def pair_report(pair, images, vit):
    ...
    for index, image_id in enumerate(pair["photo_ids"]):
        ...
        events.extend([
            dict(id=2 * index + 1, kind="tool", tool="inspect_image", status="ok", photo_id=photo_id,
                 output=images[image_id]["inspection"]),
            dict(id=2 * index + 2, kind="tool", tool="classify_breed", status="ok", photo_id=photo_id,
                 output={k: v for k, v in vit[image_id].items()
    ...
    with patch.object(evidence, "MIN_SCORE", 0), patch.object(evidence, "MIN_MARGIN", 0):
        ...
        report = evidence.build_report(case, "visual_matches" if eligible_ids else "inconclusive",
                                       eligible_ids[0] if eligible_ids else None, "registered_pair_replay")
    emitted = bool(report["candidates"])
    return dict(**pair, report=report, comparison=comparison, case=case, emitted=emitted,
...
def agreement():
    ...
    state = dict(started_at_utc=now(), complete=False, protocol_sha256=sha(DIRECTORY / "protocol.json"),
                 new_vit_rows=[], new_resnet_rows=[], controller_calls=0)
    ...
    try:
        ...
        state["resnet_baseline"] = [dict(
            image_id=r["id"], cohort=r["cohort"], emitted=resnet_reports(resnet[r["id"]]),
            correct=resnet[r["id"]].get("correct") if r["cohort"] == "supported_breed" else None,
            top1=resnet[r["id"]]["candidates"][0], margin=resnet[r["id"]]["margin"],
        ) for r in manifest["images"]]
        state["false_non_dog_pair_reports"] = [p["id"] for p in state["pairs"]
                                                if p["cohort"] == "non_dog" and p["emitted"]]
        state["false_non_dog_resnet_reports"] = [r["image_id"] for r in state["resnet_baseline"]
                                                  if r["cohort"] == "non_dog" and r["emitted"]]
        state["vision_triggered"] = bool(state["false_non_dog_pair_reports"]
                                          or state["false_non_dog_resnet_reports"])
        verify_inputs()
        ...
        if conn:
            conn.close()
        write(target, state)
    print(json.dumps({k: state[k] for k in ["complete", "false_non_dog_pair_reports",
                                          "false_non_dog_resnet_reports", "vision_triggered"]}))
...
def vision_request(protocol, row=None):
    ...
    return dict(model=protocol["model"], messages=[message], format=protocol["vision_schema"],
                stream=False, think=False, options=protocol["vision_options"], keep_alive="30m")
...
def parse_screen(payload):
    ...
    return dict(**result, accepted=accepted,
...
def vision():
    ...
    assert stage_a["complete"] and stage_a["vision_triggered"], "Registered continuation condition is not met."
    target = DIRECTORY / "vision-results.json"
    state = dict(started_at_utc=now(), complete=False, rows=[],
                 protocol_sha256=sha(DIRECTORY / "protocol.json"), stage_a_sha256=sha(DIRECTORY / "agreement-results.json"),
    ...
    with httpx.Client(trust_env=False, timeout=protocol["vision_timeout_seconds"]) as client:
        try:
            ...
            with sampler:
                state["memory_baseline"] = dict(rss_bytes=sampler.peak_rss,
                                                footprint_bytes=sampler.peak_footprint)
                for name in protocol["vision_schedule"]:
                    ...
                    if info:
                        redacted["messages"][0]["images"] = [dict(
                            local_path=info["vision_path"], sha256=info["vision_sha256"], encoding="base64 PNG"
                        )]
                    row = dict(image_id=name, request=redacted,
                               request_sha256=hashlib.sha256(wire).hexdigest(), started_at_utc=now())
                    state["rows"].append(row)
                    write(target, state)
                    started = time.monotonic()
                    try:
                        response = client.post(OLLAMA_URL + "/api/chat", content=wire,
                                               headers={"Content-Type": "application/json"})
                        row.update(seconds=time.monotonic() - started, status_code=response.status_code,
                        ...
                        raise
                    if info:
                        row.update(cohort=info["cohort"], live_dog_present=info["live_dog_present"],
                                   expected_dog_count=info["expected_dog_count"],
                                   subject_type_correct=row["screen"].get("subject_type") == info["expected_subject_type"],
            ...
            state["memory"]["scope_note"] = "Isolated screen runner plus project Ollama; classifiers unloaded. Not a whole-app concurrent pipeline measurement."
            verify_inputs()
...
def verify():
    ...
    out = dict(verified_at_utc=now(), source_and_input_hashes_unchanged=True,
    ...
    if a["vision_triggered"]:
        ...
        summary = dict(
            ...
            live_dog_total=17,
            supported_reports=sum(r["resnet_screened_emitted"] for r in rows if r["cohort"] == "supported_breed"),
            supported_total=12,
            wrong_supported_reports=sum(r["resnet_screened_emitted"] and not r["baseline_correct"]
                                        for r in rows if r["cohort"] == "supported_breed"),
            screened_non_dog_reports=sum(r["resnet_screened_emitted"] for r in rows if not r["live_dog_present"]),
            multi_dog_individual_scopes=sum(r["screen"]["proposed_scope"] == "individual"
                                            for r in rows if r["expected_dog_count"] == "multiple"),
            valid_calls=sum("error" not in r for r in v["rows"]), planned_calls=26,
            no_image_accepted=v["rows"][-1]["screen"]["accepted"],
            prevented_resnet_non_dog_reports=sum(r["resnet_baseline_emitted"] and not r["resnet_screened_emitted"]
                                                  for r in rows if not r["live_dog_present"]),
        )
        summary["advance_criteria_met"] = (
            summary["non_dog_accepts"] == 0 and summary["live_dog_accepts"] >= 16
            ...
            and summary["valid_calls"] == 26 and not summary["no_image_accepted"]
        )
        ...
        out["decision"] = "No observed rejection failure under the registered trigger; stop without vision inference."
    with httpx.Client(trust_env=False, timeout=10) as client:
