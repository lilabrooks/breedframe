...
from scripts.evaluate_suitability import check_idle, pair_report, read, resnet_reports, resnet_worker, sha, write
...
def prepare():
    ...
    for case in sorted(cases, key=lambda c: c["id"]):
        ...
        for name in case["photo_ids"]:
            ...
            images.append(dict(
                id=name, case_id=case["id"], path=str(source.relative_to(ROOT)), sha256=sha(source),
                normalized_path=str(path.relative_to(ROOT)), normalized_sha256=sha(path),
                normalized_metadata=metadata, inspection=inspect_image(path),
    ...
    assert not ({r["normalized_metadata"]["sha256"] for r in images}
    ...
    manifest = dict(created_at_utc=now(), cases=cases, images=images,
                    first_photo_assignment="Python Random(42), shuffle each pair in sorted case-id order",
                    exposure=selection["exposure"], exclusions=selection["exclusions"],
                    previous_subjects=sorted(old_ids))
    write(DIRECTORY / "manifest.json", manifest, exclusive=True)
    schedule = sorted(cases, key=lambda c: ({"non_dog": 0, "supported_breed": 1}.get(c["cohort"], 2), c["id"]))
    ...
    files = [Path(__file__).resolve(), ROOT / "scripts/evaluate_suitability.py",
             ROOT / "scripts/offline.sb", ROOT / "uv.lock", DIRECTORY / "selection.json",
    ...
    for model in ["vit", "resnet50-candidate"]:
        files.extend(ROOT / "models" / model / n for n in ["config.json", "preprocessor_config.json", "model.safetensors"])
    write(DIRECTORY / "protocol.json", dict(
        ...
        schedule=calls, classifier_timeout_seconds=60, controller_calls=0, vision_calls=0,
        ...
        primary_criteria=dict(supported_cases=6, min_reports=3, max_error_fraction=0.1,
                              non_dog_cases=3, max_non_dog_reports=0),
...
def account(case, model, emitted, top1, raw):
    ...
    if case["cohort"] == "supported_breed" and emitted:
        correct = top1["class_id"] == case["expected_native_id"] if model == "resnet" else label == case["expected_label"]
    ...
    return dict(case_id=case["id"], cohort=case["cohort"], model=model, emitted=emitted,
                top1=top1, correct=correct, scope_obligation=scope, evidence=raw)
...
def evaluate(manifest, calls):
    ...
    for c in manifest["cases"]:
        ...
        rows.append(account(c, "resnet", emitted, r["candidates"][0] if emitted else None,
                            dict(image_id=c["initial_photo_id"], native_top1=r["candidates"][0],
                                 score=r["top1_score"], margin=r["margin"])))
        p = pair_report(c, images, vit)
        rows.append(account(c, "vit", p["emitted"], p["report"]["candidates"][0] if p["emitted"] else None,
    ...
    for model in ["resnet", "vit"]:
        ...
        for cohort in sorted({c["cohort"] for c in manifest["cases"]}):
            ...
            if cohort == "supported_breed":
                g.update(correct=sum(r["correct"] is True for r in reports),
                         wrong=sum(r["correct"] is False for r in reports),
                         coverage=len(reports) / len(subset),
                         error_fraction=sum(r["correct"] is False for r in reports) / len(reports) if reports else None)
            cohorts[cohort] = g
        supported, negatives = cohorts["supported_breed"], cohorts["non_dog"]
        assert supported["cases"] == 6 and negatives["cases"] == 3
        groups[model] = dict(cohorts=cohorts, primary_targets_met=(
            supported["reports"] >= 3 and supported["wrong"] / max(supported["reports"], 1) <= 0.1
            and negatives["reports"] == 0), product_ready=False)
    return dict(rows=rows, models=groups, scope_note="Annotations diagnose scope after inference; they never gate proposals.")


def run():
    protocol, manifest = verify_inputs()
    target = DIRECTORY / "results.json"
    state = dict(started_at_utc=now(), complete=False, calls=[], controller_calls=0, vision_calls=0,
...
def verify():
    ...
    assert [{k: r[k] for k in ["model", "case_id", "image_id"]} for r in result["calls"]] == protocol["schedule"]
    ...
    expected = {str(r["expected_native_id"]): r["expected_label"] for r in manifest["cases"] if r["cohort"] == "supported_breed"}
    ...
    write(DIRECTORY / "verification.json", dict(
        verified_at_utc=now(), results_sha256=sha(DIRECTORY / "results.json"),
        source_and_input_hashes_unchanged=True, all_39_calls_accounted_for=True,
        saved_reports_and_metrics_reproduced=True, app_after=health,
    ), exclusive=True)
    print("Verified 39 calls, 26 candidate case outcomes, saved reports, denominators, and unchanged app inputs.")
